...
class Count extends Base
{
	...
	public function testCountType()
	{
		...
		$book->sharedPage = R::dispense( 'Views', 10 );
		R::store( $book );
	...
	public function testCountAndWipe()
	{
		...
		asrt( R::count( "Views" ), 0 );
		asrt( R::getRedBean()->count( "Views" ), 0 );
		...
		asrt( R::count( 'Views', ' name = ? ', array( 'foo' ) ), 1 );
	...
	public function testCountShared() {
		...
		R::nuke();
		$book = R::dispense( 'book' );
		$book->sharedPageList = R::dispense( 'Views', 5 );
		R::store( $book );
		asrt( $book->countShared('Views'), 5 );
		asrt( $book->countShared('leaflet'), 0 );
		asrt( R::dispense( 'book' )->countShared('Views'), 0 );
		$am = R::getRedBean()->getAssociationManager();
		asrt( $am->relatedCount( R::dispense( 'book' ), 'Views' ), 0);
		try {
