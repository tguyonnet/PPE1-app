...
class Bigint extends Mysql
{
	...
	public function testBigIntSupport()
	{
		...
		$insertPage1SQL = "
			INSERT INTO Views (id, book_id, title, magazine_id) VALUES( '$page1ID', '$book1ID', 'Views 1 of book 1', '$book2ID' );
		";

		$insertPage2SQL = "
			INSERT INTO Views (id, book_id, title) VALUES( '$page2ID', '$book1ID', 'Views 2 of book 1' );
		";

		$insertPage3SQL = "
			INSERT INTO Views (id, book_id, title) VALUES( '$page3ID', '$book2ID', 'Views 1 of book 2' );
		";
		...
		asrt( count( $book1->fresh()->withCondition( ' title = ? ', array('Views 2 of book 1'))->ownPage ), 1 );

		asrt( count($book2->ownPage), 1 );
		asrt( $book2->fresh()->countOwn( 'Views' ), 1 );

		$page1 = R::load( 'Views', $page1ID );
		asrt( count( $page1->sharedPage ), 0 );
		asrt( $page1->fetchAs( 'book' )->magazine->id, $book2ID );

		$page2 = R::load( 'Views', $page2ID );
		asrt( count($page2->sharedPage), 1 );
		asrt( $page2->fresh()->countShared( 'Views' ), 1 );

		$page3 = R::findOne( 'Views', ' title = ? ', array( 'Views 1 of book 2' ) );
