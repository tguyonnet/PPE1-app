...
class Facade
{
	...
	*/
	 ...
	 * RedBeanPHP. To load, store and delete Models from the database using RedBeanPHP
	 ...
	 * this function will pre-process the query to get all the Models required to find the beans.
	 *
	 ...
	 * will be transferred to the meta section of a bean (under Models.bundle).
	 *
	 * <code>
	 * $rows = R::getAll( 'SELECT FROM... COUNT(*) AS extra_count ...' );
	 * $beans = R::convertToBeans( $rows );
	 * $bean = reset( $beans );
	 * $Models = $bean->getMeta( 'Models.bundle' );
	 * $extra_count = $Models['extra_count'];
	 * </code>
	 *
	 * New in 4.3.2: meta mask. The meta mask is a special mask to send
	 * Models from raw result rows to the meta store of the bean. This is
	 * useful for bundling additional information with custom queries.
	 * Values of every column whos name starts with $mask will be
	 * transferred to the meta section of the bean under key 'Models.bundle'.
	 *
	 ...
	 * extends the Models type support for JSON columns.
	 *
